...
input_file = "CMEMS_BIO_DOWNLOAD.dat"

url="http://nrtcmems.mercator-ocean.fr/motu-web/Motu"
product="GLOBAL_ANALYSIS_FORECAST_BIO_001_014-TDS"
dataset="dataset-global-analysis-forecast-bio-001-014"
#####################################################
def read_date():
	global initial_date
	global end_date
...
def write_download_file():
	...
	with open(file_name,"w") as file:
		file.write(python_bin_directory+" "+motu_python_script_directory+" -u "+user+" -p "+password+" -m "+url+" -s "+product+" -d "+dataset+ 
			        " -x "+lon_min+" -X "+lon_max+" -y "+lat_min+" -Y "+lat_max+" -t "+'"'+str(initial_date.strftime("%Y-%m-%d "))+" 12:00:00"+'"'+" -T "+
					'"'+str(end_date.strftime("%Y-%m-%d "))+" 12:00:00"+'"'+" -z "+start_depth+" -Z "+end_depth+" -v CHL -v PHYC -v O2 -v NO3 -v PO4 -v Si -v Fe -v PP -o "
					+output_directory+" -f "+output_file_name+".nc")	
