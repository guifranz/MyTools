...
#GFS resolution for forecast mode (0.25 or 0.5)
gfs_res = 0.25
	...
	if gfs_type == 1:
		...
		if forecast_mode == 1:
			...
			if gfs_res == 0.25:
				...
				path = url_forecast_mode+"/gfs."+str(folder_date.strftime("%Y%m%d"))+"/12/"
				
				start_day = next_start_date + datetime.timedelta(days = -1)
				
				step = start_day - folder_date
								
				hour_fcst = 11 + 24*step.days
				
				for t in range (0,27):
					
					times = "gfs.t12z.pgrb2.0p25.f" + str(hour_fcst).rjust(3,'0')

					with open(file_name,"w") as file:
						file.write("set url_inv="+path+times+".idx\n")
						file.write("set url_grb="+path+times+"\n")
						file.write('PERL get_inv.pl %url_inv% | egrep "(:TMP:2 m above|:UGRD:10 m above|:VGRD:10 m above|:RH:2 m above|:PRMSL:|:LAND:|:TCDC:entire atmosphere|:DLWRF:surface|:DSWRF:surface|:PRATE:surface|:HPBL:surface|:ALBDO:surface)" | PERL get_grib.pl %url_grb% '+str(folder_date.strftime("%Y%m%d"))+times+"_temp."+grib+"\n")

					output = subprocess.call([file_name])
										
					hour_fcst = hour_fcst + 1
		
			if gfs_res == 0.5:

				folder_date = datetime.datetime.utcnow() + datetime.timedelta(days = -1)
				
				path = url_forecast_mode+"/gfs."+str(folder_date.strftime("%Y%m%d"))+"/18/"
				
				step = next_start_date - folder_date
			...
			# if gfs_res == 0.25:
				...
				# for t in range (0,27):
					...
					# times = "gfs.t18z.pgrb2.0p25.f" + str(hour).rjust(3,'0') 
